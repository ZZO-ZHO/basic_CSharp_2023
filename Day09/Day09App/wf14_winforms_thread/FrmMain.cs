...
namespace wf14_winforms_thread
{
    public partial class FrmMain : Form
    {
        ...
        {
            BackgroundWorker worker = sender as BackgroundWorker;   // 얕은복사 Worker랑 주소 참조

            e.Result = Fibonacci((int)e.Argument, worker, e);
        }

        // 백그라운드 스레드 테스크 종료한뒤 처리

        private long Fibonacci(int arg, BackgroundWorker worker, DoWorkEventArgs e)
        {
            // arg 는 0~91보다크면 long 오버플로우 발생
            if (arg < 0|| arg>91)
            {
            ...
            else
            {
                ...
                {
                    // arg만 계산에 필요한 매개변수, worker, e 중간에 필요한 Background 복사본
        ...
        {
            PgbCalculator.Value = e.ProgressPercentage;
        }

        private void BtnStart_Click(object sender, EventArgs e)
        {
            Button button = sender as Button;
            BtnStart.Enabled = false;       // 종료시까지 사용불가
